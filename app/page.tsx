"use client"

import { useState, useEffect } from "react"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Code, TrendingUp, ArrowRight } from "lucide-react"
import Image from "next/image"

export default function LandingPage() {
  const [mounted, setMounted] = useState(false)

  useEffect(() => {
    setMounted(true)
  }, [])

  if (!mounted) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-black">
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Loading...</p>
        </div>
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-white dark:bg-black">
      {/* Background Effects */}
      <div className="absolute inset-0">
        <div
          className="absolute top-1/2 left-1/2 w-[600px] h-[600px] -translate-x-1/2 -translate-y-1/2 opacity-15"
          style={{
            background: "radial-gradient(circle, #0066ff 0%, transparent 70%)",
            filter: "blur(80px)",
          }}
        />
      </div>

      {/* Header */}
      <header className="relative z-10 pt-8 pb-4">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Image 
            src="/logo.png" 
            alt="Portfolio Logo" 
            width={160} 
            height={50} 
            className="h-12 w-auto object-contain mx-auto mb-4" 
            priority
          />
        </div>
      </header>

      {/* Main Content */}
      <main className="relative z-10 flex items-center justify-center min-h-[calc(100vh-200px)]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h1 className="text-5xl md:text-7xl font-bold text-gray-900 dark:text-white mb-6">
              Hi, I'm <span className="text-[#0066ff]">Yash</span>
            </h1>
            <p className="text-xl md:text-2xl text-gray-600 dark:text-gray-300 mb-12 max-w-3xl mx-auto">
              Choose your path to explore my expertise and journey
            </p>
          </div>

          {/* Portfolio Options */}
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {/* Web3 Marketer Card */}
            <Card className="group hover:shadow-2xl transition-all duration-500 hover:-translate-y-4 border-2 hover:border-[#0066ff]/30 bg-white/80 dark:bg-black/80 backdrop-blur-xl">
              <CardContent className="p-8 text-center">
                <div className="mb-6">
                  <div className="w-20 h-20 bg-[#0066ff]/10 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-[#0066ff]/20 transition-colors duration-300">
                    <TrendingUp className="w-10 h-10 text-[#0066ff]" />
                  </div>
                  <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">
                    Web3 Marketer
                  </h2>
                  <p className="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
                    Driving Web3 growth by blending strategy, storytelling, and community magic. 
                    Explore my marketing expertise and successful campaigns.
                  </p>
                </div>
                
                <div className="space-y-3 mb-8">
                  <div className="flex items-center justify-center text-sm text-gray-600 dark:text-gray-400">
                    <span className="w-2 h-2 bg-[#0066ff] rounded-full mr-2"></span>
                    Social Media Growth
                  </div>
                  <div className="flex items-center justify-center text-sm text-gray-600 dark:text-gray-400">
                    <span className="w-2 h-2 bg-[#0066ff] rounded-full mr-2"></span>
                    Community Management
                  </div>
                  <div className="flex items-center justify-center text-sm text-gray-600 dark:text-gray-400">
                    <span className="w-2 h-2 bg-[#0066ff] rounded-full mr-2"></span>
                    DeFi & Web3 Expertise
                  </div>
                </div>

                <Button
                  size="lg"
                  className="w-full bg-[#0066ff] hover:bg-[#0052cc] text-white py-3 text-lg font-medium transition-all duration-200 hover:scale-105 group-hover:shadow-lg"
                  onClick={() => window.location.href = '/marketer'}
                >
                  Explore Marketing Portfolio
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </CardContent>
            </Card>

            {/* Programmer Card */}
            <Card className="group hover:shadow-2xl transition-all duration-500 hover:-translate-y-4 border-2 hover:border-[#0066ff]/30 bg-white/80 dark:bg-black/80 backdrop-blur-xl">
              <CardContent className="p-8 text-center">
                <div className="mb-6">
                  <div className="w-20 h-20 bg-[#0066ff]/10 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-[#0066ff]/20 transition-colors duration-300">
                    <Code className="w-10 h-10 text-[#0066ff]" />
                  </div>
                  <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">
                    Programmer
                  </h2>
                  <p className="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
                    Building innovative solutions with modern technologies. 
                    Discover my development projects and technical expertise.
                  </p>
                </div>
                
                <div className="space-y-3 mb-8">
                  <div className="flex items-center justify-center text-sm text-gray-600 dark:text-gray-400">
                    <span className="w-2 h-2 bg-[#0066ff] rounded-full mr-2"></span>
                    Full-Stack Development
                  </div>
                  <div className="flex items-center justify-center text-sm text-gray-600 dark:text-gray-400">
                    <span className="w-2 h-2 bg-[#0066ff] rounded-full mr-2"></span>
                    Modern Frameworks
                  </div>
                  <div className="flex items-center justify-center text-sm text-gray-600 dark:text-gray-400">
                    <span className="w-2 h-2 bg-[#0066ff] rounded-full mr-2"></span>
                    Problem Solving
                  </div>
                </div>

                <Button
                  size="lg"
                  variant="outline"
                  className="w-full border-2 border-[#0066ff] text-[#0066ff] hover:bg-[#0066ff] hover:text-white py-3 text-lg font-medium transition-all duration-200 hover:scale-105 group-hover:shadow-lg"
                  onClick={() => window.location.href = '/programmer'}
                >
                  Explore Dev Portfolio
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </CardContent>
            </Card>
          </div>
        </div>
      </main>

      {/* Footer */}
      <footer className="relative z-10 py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-gray-500 dark:text-gray-400">
            &copy; {new Date().getFullYear()} Yash Portfolio. Choose your journey.
          </p>
        </div>
      </footer>
    </div>
  )
}